...
TITM = TypeVar('Item')
TRET = TypeVar('Return')
TACC = TypeVar('Accumulator')

#=======================================================================
#=======================================================================
#=======================================================================

class ObjTyList(list):
	...
	def __add__(self, val):
		if val.__class__ == str:
			for l in self:
				if not l.__class__ == str:
					return str(self) + val
			return ', '.join(self) + val
		elif val.__class__ == list:
			return ObjTyList(list(self) + val)
		else:
			raise TypeError(f'can only concatenate ObjTyList (not "{val.__class__.__name__}") to ObjTyList')
	...
	def __radd__(self, val):
		if val.__class__ == str:
			for l in self:
				if not l.__class__ == str:
					return val + str(self)
			return val + ', '.join(self)
		elif val.__class__ == list:
			return ObjTyList(val+list(self))
		else:
			raise TypeError(f'can only concatenate ObjTyList (not "{val.__class__.__name__}") to ObjTyList')
	
	def map(self, func: Callable[[TITM], TRET]) -> List[TRET]:
		mapped: List[TRET] = ObjTyList([])
		for item in self:
	...
	def reduce(self, func: Callable[[TACC, TITM], TACC], accum: TACC = None) -> TACC:
		if accum == None:
			accum = self[0]
			list_ = self[1:]
		for item in list_:
	...
	def forEach(self, func: Callable[[TITM, int, list], TRET]) -> List[TRET]:
		for index, item in enumerate(self):
			func(item, index, self)

class ObjTyInt(int):
	
	def __new__(self, int_):
		return super().__new__(self, int_)
	
	def __add__(self, val):
		if val.__class__ in [int, bool]:
			return ObjTyInt(int(self) + val)
		elif val.__class__ == str:
			return str(self) + val
		else:
			raise TypeError(f'can only concatenate ObjTyInt (not "{val.__class__.__name__}") to ObjTyInt')
	
	def __radd__(self, val):
		if val.__class__  in [int, bool]:
			return ObjTyInt(val + int(self))
		elif val.__class__ == str:
			return val + str(self)
		else:
			raise TypeError(f'can only concatenate ObjTyInt (not "{val.__class__.__name__}") to ObjTyInt')
...
class ObjTyClassNames: #Use    # Obtiene todos los nombres de las clases en los objetos de 'Utils' y la cantidad.    Detecta Clases con Formato de primer letra Mayúscula. Ejemplo: 'PrimeroSegundoTercero'
	...
	def __init__(self, obj):
		...
		\r |
		\r |    # Nota: Estan disponibles las funciones
		\r |    # map, filter, reduce y forEach para las listas.
		\r |
		\r |    # Ejemplos:
		\r |    mapped   = utils.classes.list.map(lambda item: ...)
		\r |    filtered = utils.classes.list.filter(lambda item: ...)
		\r |    reduced  = utils.classes.list.reduce(lambda acc, item: ...)
...
class ObjTyFunctionNames: #Use # Obtiene todos los nombres de las funciones en los objetos de 'Utils' y la cantidad. Detecta funciones con Formato de primer letra minúscula. Ejemplo: 'primeroSegundoTercero'
	...
	def __init__(self, obj):
		...
		\r |
		\r |    # Ejemplos:
		\r |    mapped   = utils.functions.list.map(lambda item: ...)
		\r |    filtered = utils.functions.list.filter(lambda item: ...)
		\r |    reduced  = utils.functions.list.reduce(lambda acc, item: ...)
...
class Utils:
	...
	class Utilities:	# Funciones de utilidad para cosas generales.
		...
		def isPrime(num):												# Indica si un número es 'Primo' o no.
...
if __name__ == '__main__':
	...
	# Output:
	# ObjTyList (forEach):
	# Item 0 => '1' + '!'  -->  Next item: '2'
	# Item 1 => '2' + '!'  -->  Next item: '3'
	# Item 2 => '3' + '!'  -->  Next item: '4'
	# Item 3 => '4' + '!'  -->  Next item: '5'
	# Item 4 => '5' + '!'  -->  Next item: None
	# ['1!', '2!', '3!', '4!', '5!']
	...
	# ~ filename = utils.Actions.Explorer.getFileName()
	# ~ if filename:
